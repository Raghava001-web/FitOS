...
export const getTopSet = (log: WorkoutLog | undefined) => {
  if (!log || log.sets.length === 0) return null;
  return [...log.sets].sort((a, b) => b.weightKg === a.weightKg ? b.reps - a.reps : b.weightKg - a.weightKg)[0] ?? null;
};

export const getHistoricBestSet = (logs: WorkoutLog[], exerciseId: string) => {
  const exerciseLogs = getExerciseLogs(logs, exerciseId);
  if (!exerciseLogs.length) return null;
  let bestLog = exerciseLogs[0];
  let bestSet = getTopSet(bestLog);
  for (const log of exerciseLogs) {
    ...
    if (top.weightKg > bestSet.weightKg || (top.weightKg === bestSet.weightKg && top.reps > bestSet.reps)) {
      bestSet = top;
      bestLog = log;
    }
  }
  return { date: bestLog.date, ...bestSet! };
};

export const calculateTotalVolume = (log: WorkoutLog) => {
  return log.sets.reduce((acc, set) => acc + (set.weightKg * set.reps), 0);
};
...
export const recommendProgression = (
  ...
  if (readinessLow || metrics.stressScore > 70) {
    increment = -roundToStep(latestTopSet.weightKg * 0.1);
    action = "reduce";
    warnings.push("High stress or low readiness detected. Capping progression and steering toward a deload session to protect recovery.");
  } else if (profile.sleepHours < 6) {
    increment = 0;
    action = "hold";
    warnings.push("Sleep is compromised (< 6 hrs). Hold current weight and focus on movement quality instead of pushing loads.");
  ...
  const summary =
    increment > 0
      ...
      : increment < 0
        ? `Deload suggested: ${recommendedWeightKg} kg for ${exercise.name}.`
